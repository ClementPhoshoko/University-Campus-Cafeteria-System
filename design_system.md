# MerchantMunchies Mobile Design System
...
This design system defines the visual foundations and reusable UI tokens for MerchantMunchies.
...
These principles are based on the supplied MerchantMunchies screen direction: bright blue primary accent, white dominant surfaces, subtle blue-tinted surfaces, soft neutral grey, greyish-black typography, rounded cards, light borders, minimal shadows and generous whitespace. fileciteturn0file0L38-L101
...
MerchantMunchies uses one recognizable bright blue family. Keep the hue consistent across the whole application.
...
Cafeteria cards are a core MerchantMunchies pattern.
...
Prefix reusable components with `Cb` to clearly separate MerchantMunchies UI from Android framework components.
...
```text
MerchantMunchies Light
```

Future themes can include:

```text
MerchantMunchies Dark
...
**MerchantMunchies should feel like a calm, modern campus utility that happens to make food ordering extremely convenient.**
...
```text
WHITE + BRIGHT MerchantMunchies BLUE + SOFT GREY-BLACK TEXT
