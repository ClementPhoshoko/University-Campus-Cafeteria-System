...
export function AddVendorModal({ onClose, onSubmit, submitting = false }) {
  ...
  const [form, setForm] = useState({ name: '', description: '', logo_url: '', support_email: '', support_phone: '', corporate_catering_enabled: false, onboarding_key: '', site_id: '', building_id: '', collection_point_id: '', service_status: 'closed', estimated_prep_minutes: '15', order_cutoff_minutes: '0', collection_instructions: '' });
  ...
  const submit = async () => {
    ...
    try {
      await onSubmit({
        name: form.name.trim(), description: form.description.trim() || null, logo_url: form.logo_url.trim() || null,
