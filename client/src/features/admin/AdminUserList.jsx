...
function RoleBadge({ role }) { const meta = roleMeta[role]; return <span className={`admin-user-role admin-user-role--${meta?.tone || 'info'}`}>{meta?.label || role}</span>; }
function UserRow({ user }) { return <tr className="admin-order-row"><td><div className="admin-user-cell"><Avatar name={user.full_name || user.email} /><div className="admin-user-cell__body"><Link to={`/admin/users/${user.id}`} className="admin-user-cell__name">{user.full_name || 'Unnamed user'}</Link><span className="admin-user-cell__email">{user.email}</span></div></div></td><td><span className="admin-user-num">{user.employee_number || '—'}</span></td><td><div className="admin-user-loc"><span>{user.department || '—'}</span><span className="admin-user-loc__meta">{new Date(user.created_at).toLocaleDateString('en-ZA')}</span></div></td><td><div className="admin-user-roles">{user.roles?.length ? user.roles.map((role) => <RoleBadge key={role} role={role} />).reduce((acc, el, i) => [acc, <span key={`sep-${i}`} className="admin-user-roles__sep">,</span>, el]) : <span className="admin-user-roles__none">No roles</span>}</div></td><td><StatusPill active={user.is_active} /></td><td className="admin-order-cta"><Link to={`/admin/users/${user.id}`} className="admin-link-cta">Manage <IconChevronRight size={13} /></Link></td></tr>; }
function StatBlock({ label, value, icon: Icon, loading }) { return <div className="admin-users-kpi"><span className="admin-users-kpi__icon"><Icon size={24} /></span><div className="admin-users-kpi__body"><span className="admin-users-kpi__label">{label}</span><span className="admin-users-kpi__value">{loading ? <span className="skeleton skeleton--kpi-value" /> : value}</span></div></div>; }
...
export default function AdminUserList() {
  ...
  return <div className="admin-orders"><header className="admin-users-header"><div className="admin-users-header__info"><span className="admin-users-header__eyebrow">Identity & access</span><p className="admin-users-header__sub">Search employees, vendors and admins. Manage roles and access across the platform.</p></div></header><section className="admin-users-kpis"><StatBlock label="Users on page" value={users.length} icon={IconUsers} loading={loading} /><StatBlock label="Active on page" value={activeCount} icon={IconCheck} loading={loading} /><StatBlock label="Inactive on page" value={users.length - activeCount} icon={IconClock} loading={loading} /><StatBlock label="Admins on page" value={adminCount} icon={IconUserCircle} loading={loading} /></section><div className="admin-orders__filters"><div className="admin-vendors__search admin-orders__search"><IconSearch size={16} /><input type="search" placeholder="Search users by name, email or employee number..." value={query} onChange={(e) => setQuery(e.target.value)} /></div><div className="admin-vendors__chips">{USER_STATUS_FILTERS.map((filter) => <button key={filter.id} type="button" className={`admin-vendors__chip${statusFilter === filter.id ? ' admin-vendors__chip--active' : ''}`} onClick={() => setStatusFilter(filter.id)}>{filter.label}</button>)}</div><div className="admin-orders__vendor-select-wrapper" ref={roleDropdownRef}><button type="button" className="admin-orders__vendor-select" onClick={() => setRoleDropdownOpen((open) => !open)}> {ROLE_FILTERS.find((role) => role.id === roleFilter)?.label || 'All roles'} <IconChevronDown size={14} /></button>{roleDropdownOpen && <div className="admin-orders__vendor-select-dropdown">{ROLE_FILTERS.map((role) => <button key={role.id} type="button" className="admin-orders__vendor-select-option" onClick={() => { setRoleFilter(role.id); setRoleDropdownOpen(false); }}>{role.label}</button>)}</div>}</div></div>{loading ? <div className="admin-vendor-detail"><SkeletonTable rows={5} columns={5} /></div> : error ? <div className="admin-empty"><h3>Could not load users</h3><p>{error}</p></div> : visibleUsers.length ? <><div className="admin-card admin-card--full"><table className="admin-table"><thead><tr><th>User</th><th>Number</th><th>Department / joined</th><th>Roles</th><th>Status</th><th /></tr></thead><tbody>{visibleUsers.map((user) => <UserRow key={user.id} user={user} />)}</tbody></table></div><Pagination currentPage={page} totalPages={pagination.totalPages || 1} totalItems={pagination.total || visibleUsers.length} itemsPerPage={ITEMS_PER_PAGE} label="users" onPageChange={setPage} /></> : <div className="admin-empty"><img src={emptyStateAvatar} alt="" className="admin-empty__avatar" /><h3>No users match those filters</h3><p>Try clearing the search or changing the role filter.</p></div>}</div>;
}
