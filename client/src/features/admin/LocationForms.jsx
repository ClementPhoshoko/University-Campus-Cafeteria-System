...
function Field({ label, children, full = false, hint = '' }) {
  return <label className={`admin-modal__field${full ? ' admin-modal__field--full' : ''}`}><span>{label}</span>{children}{hint && <span className="admin-modal__hint">{hint}</span>}</label>;
}

export function LocationModal({ title, initial = {}, fields, floors = [], onClose, onSubmit, submitting }) {
  const [form, setForm] = useState(initial);
  const [error, setError] = useState('');
  const update = (key, value) => setForm((prev) => ({ ...prev, [key]: key === 'code' ? value.toUpperCase().replace(/[^A-Z0-9_-]/g, '') : value }));
  const renderFieldInput = (field) => {
    if (field.type === 'floor-select') {
      const value = form[field.key] ?? '';
      return (
        <select className="admin-input" value={String(value || '')} onChange={(e) => update(field.key, e.target.value || null)}>
          <option value="">No floor (building level)</option>
          {floors.map((floor) => (
            <option key={floor.id} value={floor.id}>
              {floor.name}{floor.level_number !== null && floor.level_number !== undefined ? ` — Level ${floor.level_number}` : ''}
            </option>
          ))}
        </select>
      );
...
export const buildingFields = [
  { key: 'name', label: 'Building name' }, { key: 'code', label: 'Building code', hint: 'Uppercase, digits, hyphens, or underscores only' }, { key: 'address', label: 'Address', full: true },
...
export const collectionPointFields = [
  { key: 'name', label: 'Collection point name' }, { key: 'floor_id', label: 'Floor', type: 'floor-select', hint: 'Optional — pick a floor in this building' }, { key: 'instructions', label: 'Instructions', type: 'textarea', full: true },
...
export const deliveryFields = [
  { key: 'name', label: 'Delivery location name' }, { key: 'floor_id', label: 'Floor', type: 'floor-select', hint: 'Optional — pick a floor in this building' }, { key: 'room_or_venue', label: 'Room or venue', full: true },
