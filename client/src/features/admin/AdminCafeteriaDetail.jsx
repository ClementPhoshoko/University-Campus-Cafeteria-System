...
export default function AdminCafeteriaDetail() {
  ...
  return <div className="admin-order-detail">
    ...
    {modal === 'edit' && !isSite && <BuildingModal initial={entity} onClose={() => setModal(null)} onSubmit={(payload) => mutate(async () => { const { cover_file: coverFile, ...entityPayload } = payload; await updateBuilding(token, entity.id, normalizePayload(entityPayload)); if (coverFile) await uploadAdminAsset(token, 'building', entity.id, coverFile); })} submitting={mutating} />}
    {modal === 'building' && <BuildingModal initial={newBuildingDefaults} onClose={() => setModal(null)} onSubmit={(payload) => mutate(async () => { const { cover_file: coverFile, ...buildingPayload } = payload; const response = await createBuilding(token, entity.id, normalizePayload(buildingPayload)); if (coverFile && response?.building?.id) await uploadAdminAsset(token, 'building', response.building.id, coverFile); })} submitting={mutating} />}
    ...
    <Link to={`/admin/cafeterias/${building.id}`} className="admin-building-card">
      <div className="admin-building-card__media">
        {building.cover_image_url ? (
          <SmartImage src={building.cover_image_url} alt={building.name} width={160} height={160} />
        ) : (
