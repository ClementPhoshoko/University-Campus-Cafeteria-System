...
import { buildingFieldRows, buildingFields, collectionPointFields, deliveryFields, floorFields, LocationModal, newBuildingDefaults, siteFields } from './LocationForms.jsx';
...
export default function AdminCafeteriaDetail() {
  ...
  const heroAddress = entity.street_address
    ? [entity.street_address, entity.city, entity.province].filter(Boolean).join(', ')
    : entity.address || 'No address recorded';
  ...
  return <div className="admin-order-detail">
    ...
    </div><aside className="admin-order-grid__side"><section className="admin-card"><header className="admin-card__head"><div><span className="admin-card__eyebrow">Profile</span><h3 className="admin-card__title">{isSite ? 'Site details' : 'Building details'}</h3></div></header><div className="admin-vendor-section"><Info label="Code" value={entity.code || '—'} /><Info label="Name" value={entity.name} /><Info label="Address" value={entity.address || '—'} />{entity.street_address && <Info label="Street" value={entity.street_address} />}{entity.city && <Info label="City" value={entity.city} />}{entity.province && <Info label="Province" value={entity.province} />}{entity.postal_code && <Info label="Postal code" value={entity.postal_code} />}{entity.country && <Info label="Country" value={entity.country} />}{isSite && <Info label="Timezone" value={entity.timezone || '—'} />}{entity.latitude && entity.longitude && <Info label="Coordinates" value={`${entity.latitude}, ${entity.longitude}`} />}</div></section><section className="admin-card"><header className="admin-card__head"><div><span className="admin-card__eyebrow">Vendors</span><h3 className="admin-card__title">Operating here</h3></div></header>{vendors.length ? <ChildRows items={vendors} type="vendors" /> : <p className="admin-vendor-empty-copy">No vendors assigned.</p>}</section></aside></div>
    {modal === 'edit' && isSite && <NewSiteModal initial={entity} onClose={() => setModal(null)} onSubmit={(payload) => mutate(async () => { const { cover_file: coverFile, ...entityPayload } = payload; await updateSite(token, entity.id, normalizePayload(entityPayload)); if (coverFile) await uploadAdminAsset(token, 'site', entity.id, coverFile); })} submitting={mutating} />}
    {modal === 'edit' && !isSite && <LocationModal title="Edit building" initial={entity} fields={buildingFields} rows={buildingFieldRows} onClose={() => setModal(null)} onSubmit={(payload) => mutate(async () => { const { cover_file: coverFile, ...entityPayload } = payload; await updateBuilding(token, entity.id, normalizePayload(entityPayload)); if (coverFile) await uploadAdminAsset(token, 'building', entity.id, coverFile); })} submitting={mutating} />}
    {modal === 'building' && <LocationModal title="Add building" initial={newBuildingDefaults} fields={buildingFields} rows={buildingFieldRows} onClose={() => setModal(null)} onSubmit={(payload) => mutate(async () => { const { cover_file: coverFile, ...buildingPayload } = payload; const response = await createBuilding(token, entity.id, normalizePayload(buildingPayload)); if (coverFile && response?.building?.id) await uploadAdminAsset(token, 'building', response.building.id, coverFile); })} submitting={mutating} />}
...
function BuildingPreview({ building }) {
  const displayAddress = building.street_address
    ? [building.street_address, building.city, building.province].filter(Boolean).join(', ')
    ...
    <Link to={`/admin/cafeterias/${building.id}`} className="admin-building-card">
      ...
      <div className="admin-building-card__body">
        <div className="admin-building-card__head">
          <span className="admin-building-card__code">{building.code || 'BUILDING'}</span>
          <StatusPill active={building.is_active} />
        </div>
        <h4 className="admin-building-card__name">{building.name}</h4>
        <p className="admin-building-card__site"><IconMapPin size={10} />{displayAddress}</p>
        <div className="admin-building-card__stats">
          <span><strong>{building.floor_count || 0}</strong> floors</span>
          <span><strong>{building.collection_point_count || 0}</strong> pickup</span>
        </div>
      </div>
    </Link>
  );
