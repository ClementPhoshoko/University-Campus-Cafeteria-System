...
import {
  createBuilding, createCollectionPoint, createDeliveryLocation, createFloor, getBuilding, getSite, listBuildings, listBuildingVendors, listCollectionPoints, listDeliveryLocations, listFloors, updateBuilding, updateCollectionPoint, updateDeliveryLocation, updateFloor, updateSite,
...
export default function AdminCafeteriaDetail() {
  ...
  return <div className="admin-order-detail">
    ...
    {modal === 'edit' && <LocationModal title={isSite ? 'Edit site' : 'Edit building'} initial={entity} fields={isSite ? siteFields : buildingFields} onClose={() => setModal(null)} onSubmit={(payload) => mutate(() => isSite ? updateSite(token, entity.id, normalizePayload(payload)) : updateBuilding(token, entity.id, normalizePayload(payload)))} submitting={mutating} />}
...
function BuildingPreview({ building }) { return <Link to={`/admin/cafeterias/${building.id}`} className="admin-building-card"><div className="admin-building-card__media"><div className="admin-building-card__placeholder"><IconBuilding size={22} /></div></div><div className="admin-building-card__body"><div className="admin-building-card__head"><span className="admin-building-card__code">{building.code || 'BUILDING'}</span><StatusPill active={building.is_active} /></div><h4 className="admin-building-card__name">{building.name}</h4><p className="admin-building-card__site"><IconMapPin size={10} />{building.address || 'No address recorded'}</p><div className="admin-building-card__stats"><span><strong>{building.floor_count || 0}</strong> floors</span><span><strong>{building.collection_point_count || 0}</strong> pickup</span></div></div></Link>; }
